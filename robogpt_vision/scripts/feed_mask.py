...
class ObjectDetector:
    def __init__(self, boundary_rect=(490, 200, 300, 300), image_topic="/camera/color/image_raw"):
        ...
        self.image_pub = rospy.Publisher("/web_feed",Image,queue_size=10)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.latest_frame is not None:
                ...
                masking = rospy.get_param("/masking",default="off")
                if masking:
...
if __name__ == "__main__":
    ...
    detector = ObjectDetector(image_topic="/camera/color/image_raw")
    detector.run()
