...
class YoloV5Detection:
    ...
    '''
    def __init__(self):
        self.weights_path = 'core/skills/vision_skills/object_detection/weights/yolov5/best.pt'
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path=self.weights_path, force_reload=True, device ='cpu')
    ...
    def detect(self, image):
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            label = self.model.names[int(cls)]  # Get class name from custom model
    ...
    def detect_objects(self, color_frame, depth_frame):
        ...
        for detection in detections:
            ...
            #Storing results
            detection_result = {"detected_object": f"{detection['class']}", "confidence": f"{detection['confidence']*100}", "center_pt": center, "depth_at_center": depth,"area":area,"xyxy":[[xmin,ymin,zmin],[xmax,ymax,zmax]], "effective_depth": effective_depth}
